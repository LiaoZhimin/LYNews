...
class  ArticleAdmin(admin.ModelAdmin):  
    list_display = ('id','name', 'category',  'excerpt', 'views','created_time')  # 表格显示的列，model的属性，多对多的列不行
    list_per_page = 50  # list_per_page设置每页显示多少条记录，默认是100条
    ordering = ('-id',)  # ordering设置默认排序字段，负号表示降序排序
    # list_editable 设置默认可编辑字段，在列表里就可以编辑
    list_editable = [ 'name','category', 'excerpt']     
    # fk_fields = ['category']  # fk_fields 设置显示外键字段
    list_display_links = ['id']  # 设置哪些字段可以点击进入编辑界面,不能在list_editable内存在
